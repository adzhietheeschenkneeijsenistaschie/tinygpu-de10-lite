...
module controller #(
    ...
    // FIX: Loop variables and found flags declared outside always block.
    // Quartus does not allow variable declarations inside procedural blocks.
    // 'int' replaced with 'integer' for broader Quartus compatibility.
    integer i, j;
    reg read_found, write_found;
    ...
    always @(posedge clk) begin
        if (reset) begin 
            ...
            // FIX: Unpacked arrays cannot be assigned a scalar 0.
            // Each element must be reset individually.
            ...
            for (i = 0; i < NUM_CHANNELS; i = i + 1) begin 
                case (controller_state[i])
                    IDLE: begin
                        // While this channel is idle, cycle through consumers looking
                        // for one with a pending request.
                        // FIX: 'break' is not synthesizable in Quartus.
                        // Replaced with found flags that suppress further assignments
                        // once the first pending request is picked up.
                        // Functionally identical: only the first eligible consumer
                        // per channel per cycle is served.
                        read_found  = 1'b0;
                        write_found = 1'b0;
                        for (j = 0; j < NUM_CONSUMERS; j = j + 1) begin 
                            if (!read_found && !write_found &&
                                consumer_read_valid[j] && !channel_serving_consumer[j]) begin 
                                ...
                                // Once we find a pending request, pick it up with
                                // this channel and stop looking for requests
                                read_found = 1'b1;
                            end else if (!read_found && !write_found &&
                                         consumer_write_valid[j] && !channel_serving_consumer[j]) begin 
                                channel_serving_consumer[j] = 1;
                                current_consumer[i] <= j;
